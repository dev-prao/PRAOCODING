...
// 초등학생(8~13) / 중학생(14~16) / 고등학생(17~19)으로 나누면?
	...
	// Up and Down
	srand(time(NULL));
	int num = rand() % 100 + 1; // 1 ~ 100까지
	//printf("숫자 : %d\n", num);
	int answer = 0; // 정답
	int chance = 5; // 기회
	while (1) // 1은 참, 0은 거짓이기에 while(1)을 사용할 시 while 문 안의 모든 구문 수행
			  // 또는 while (chance > 0)을 사용 가능
	{
		printf("남은 기회는 %d 번\n", chance--);
